...
```{r}
data.work.merged2 = na.omit(data.work.merged)
```
...
```{r}
data.work.final = select(data.work.merged2, -survey_date_month, -fin_situ_change,-survey_num, -unid)
```
...
data.work.final[,cols] <- lapply(data.work.final[,cols],as.factor)
...
set.seed(3456)
trainIndex <- createDataPartition(df$moreThanSixMths, p = .7, 
                                  list = FALSE, 
                                  times = 1)
...
model <- train(moreThanSixMths~., data = dfTrain, method = "glm", trControl = TRAINCONTROL)
model
```
